...
from services.report_templates import build_comparison_report, filtered_metric_deltas
...
class ComparisonReportTests(unittest.TestCase):
    ...
    def test_filtered_metric_deltas_transport_only(self) -> None:
        left_metadata = {
            "transport_estimated_delivery_days": 4.0,
            "transport_daily_movable_capacity_kg": 8000.0,
            "cost_total": 100000.0,
            "personnel_required_count": 40.0,
        }
        right_metadata = {
            "transport_estimated_delivery_days": 6.0,
            "transport_daily_movable_capacity_kg": 9500.0,
            "cost_total": 120000.0,
            "personnel_required_count": 42.0,
        }

        deltas = filtered_metric_deltas(left_metadata, right_metadata, "Transport")

        self.assertEqual(
            deltas,
            [
                ("transport_daily_movable_capacity_kg", 1500.0),
                ("transport_estimated_delivery_days", 2.0),
            ],
        )

    def test_comparison_report_respects_metric_filter(self) -> None:
        ...
        right_analysis = self._analysis(
            ...
            metadata={
                "transport_estimated_delivery_days": 6.0,
                "transport_daily_movable_capacity_kg": 9000.0,
                "cost_total": 130000.0,
            },
        )
        ...
        self.assertIn("- Metric filter: Transport", report)
        self.assertIn("- transport_estimated_delivery_days: +2.00", report)
        self.assertIn("- transport_daily_movable_capacity_kg: +1000.00", report)
        self.assertNotIn("- cost_total:", report)

    def test_comparison_report_shows_no_match_fallback(self) -> None:
        left_scenario = self._scenario("scenario-a", "Scenario A", "baseline")
        right_scenario = self._scenario("scenario-b", "Scenario B", "branch")
        left_analysis = self._analysis(
            critical=78.0,
